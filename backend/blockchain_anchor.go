...
func recordBlockchainAnchor(c *gin.Context) {
	...
	if err := db.Collection("verification_reports").FindOne(context.Background(), bson.M{"property_id": req.PropertyID, "sha256": req.ReportHash}).Decode(&report); err != nil {
	...
	res, err := db.Collection("properties").UpdateOne(
		context.Background(),
		bson.M{"property_id": req.PropertyID, "latest_verification_report_hash": req.ReportHash},
		bson.M{"$set": bson.M{
	...
	if res.MatchedCount == 0 {
		c.JSON(http.StatusConflict, gin.H{"error": "Property report hash does not match its latest verification artifact"})
		return
	}

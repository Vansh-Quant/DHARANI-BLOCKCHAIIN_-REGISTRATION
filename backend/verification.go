...
func sourceAuthorityRequired(c *gin.Context) bool {
	...
	if role != "AUTHORITY" && role != "ADMIN" { c.JSON(http.StatusForbidden, gin.H{"error":"Authority role required"}); return false }
	return true
}
...
func createSourceRecord(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&record); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error":"Invalid source record","details":err.Error()}); return }
	...
	if record.PropertyID == "" || record.SourceType == "" || record.SourceRecordID == "" { c.JSON(http.StatusBadRequest, gin.H{"error":"property_id, source_type and source_record_id are required"}); return }
	if record.CapturedAt.IsZero() { record.CapturedAt = time.Now().UTC() }
	var property bson.M
	if err := db.Collection("properties").FindOne(context.Background(), bson.M{"property_id":record.PropertyID}).Decode(&property); err != nil { c.JSON(http.StatusNotFound, gin.H{"error":"Property not found"}); return }
	...
	if err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error":"Could not save source record"}); return }
...
func runVerification(c *gin.Context) {
	...
	if len(records) < 2 { status="INCONCLUSIVE"; summary="At least two independent source records are required for cross-source reconciliation." } else if len(conflicts)>0 { status="CONFLICT"; summary=fmt.Sprintf("%d verification conflict(s) require authority review.",len(conflicts)) }
	report := VerificationReport{ReportVersion:"1.0",PropertyID:propertyID,GeneratedAt:time.Now().UTC(),SourceCount:len(records),Sources:records,Conflicts:conflicts,Status:status,Summary:summary}
...
func getVerificationReport(c *gin.Context) {
	propertyID := strings.TrimSpace(c.Param("propertyId")); var report VerificationReport
	err := db.Collection("verification_reports").FindOne(context.Background(), bson.M{"property_id":propertyID}, options.FindOne().SetSort(bson.D{{Key:"generated_at",Value:-1}})).Decode(&report)
	if err != nil { c.JSON(http.StatusNotFound, gin.H{"error":"Verification report not found"}); return }
...
func evaluateVerificationRules(records []SourceRecord) []Conflict {
	...
	var overlapSources []string;for _,r:=range records{if r.OverlapFlag{overlapSources=append(overlapSources,r.SourceType)}};if len(overlapSources)>0{conflicts=append(conflicts,Conflict{RuleCode:"DUPLICATE_OVERLAP",Severity:"HIGH",Title:"Parcel overlap detected",Description:"At least one source explicitly flags a duplicate or spatial overlap risk.",Sources:overlapSources})}
	var lineageSources []string;for _,r:=range records{status:=strings.ToUpper(strings.TrimSpace(r.LineageStatus));if status=="BROKEN"||status=="MISSING"||status=="INVALID"{lineageSources=append(lineageSources,r.SourceType)}};if len(lineageSources)>0{conflicts=append(conflicts,Conflict{RuleCode:"BROKEN_SUBDIVISION_LINEAGE",Severity:"MEDIUM",Title:"Subdivision lineage issue",Description:"A source reports missing or broken parent/subdivision lineage.",Sources:lineageSources})}
	var missingSources []string;for _,r:=range records{enc:=strings.ToUpper(strings.TrimSpace(r.EncumbranceStatus));lit:=strings.ToUpper(strings.TrimSpace(r.LitigationStatus));if enc==""||lit==""||enc=="UNKNOWN"||lit=="UNKNOWN"||enc=="MISSING"||lit=="MISSING"{missingSources=append(missingSources,r.SourceType)}};if len(missingSources)>0{conflicts=append(conflicts,Conflict{RuleCode:"MISSING_RISK_DATA",Severity:"MEDIUM",Title:"Encumbrance/litigation data incomplete",Description:"One or more sources do not provide a usable encumbrance or litigation status.",Sources:missingSources})}
	return conflicts
}

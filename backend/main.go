...
func main() {
	...
	if err = client.Ping(context.Background(), nil); err != nil {
		log.Fatalf("MongoDB unavailable: %v", err)
	}
...
func registerUser(c *gin.Context) {
	...
	if c.BindJSON(&req) != nil || strings.TrimSpace(req.PhoneNumber) == "" {
		c.JSON(400, gin.H{"error": "Valid phone number required"})
		return
	}
	role := "CITIZEN"
	if strings.EqualFold(strings.TrimSpace(req.Email), "authority@dharani.local") || strings.EqualFold(strings.TrimSpace(req.Email), "admin@dharani.local") {
		role = "AUTHORITY"
	}
	...
	// Demo OTP remains fixed so the college team can test without an SMS provider.
...
func verifyOTP(c *gin.Context) {
	...
	if req.OTP != "1234" {
		c.JSON(401, gin.H{"error": "Invalid demo OTP"})
		return
	}
	var user bson.M
	err := db.Collection("users").FindOne(context.Background(), bson.M{"phone_number": req.PhoneNumber}).Decode(&user)
	if err != nil {
		c.JSON(404, gin.H{"error": "User not registered"})
		return
	}
	token := randomToken()
	sessionMu.Lock()
	sessions[token] = req.PhoneNumber
	sessionMu.Unlock()
...
func authMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		sessionMu.RLock()
		phone, ok := sessions[token]
		sessionMu.RUnlock()
		if !ok {
			c.JSON(401, gin.H{"error": "Session expired or invalid"})
			c.Abort()
			return
		}
		c.Set("phone", phone)
...
func registerProperty(c *gin.Context) {
	...
	if c.BindJSON(&req) != nil || req.PropertyName == "" || req.Address == "" || req.Area <= 0 {
		c.JSON(400, gin.H{"error": "property_name, address and positive area are required"})
		return
	}
	id := makePropertyID()
	...
	_, err := db.Collection("properties").InsertOne(context.Background(), doc)
	if err != nil {
		c.JSON(500, gin.H{"error": "Could not save property"})
		return
	}
...
func getUserProperties(c *gin.Context) {
	...
	var docs []bson.M
	if cur.All(context.Background(), &docs) != nil {
		c.JSON(500, gin.H{"error": "Database error"})
		return
	}
...
func searchProperties(c *gin.Context) {
	...
	if q == "" {
		c.JSON(200, gin.H{"properties": []bson.M{}, "count": 0})
		return
	}
	...
	if err != nil { // fallback regex for local Mongo text-index differences
		rx := primitive.Regex{Pattern: q, Options: "i"}
		cur, err = db.Collection("properties").Find(context.Background(), bson.M{"$or": []bson.M{{"property_name": rx}, {"address": rx}, {"location": rx}, {"property_id": rx}}}, options.Find().SetLimit(30))
	}
	if err != nil {
...
func getPropertyDetail(c *gin.Context) {
	var doc bson.M
	if err := db.Collection("properties").FindOne(context.Background(), bson.M{"property_id": c.Param("id")}).Decode(&doc); err != nil {
...
func getPropertyPassport(c *gin.Context) {
	...
	doc["qr_payload"] = makeQRPayload(c.Param("id"))
...
func addDocument(c *gin.Context) {
	...
	if c.BindJSON(&req) != nil || req.Name == "" {
		c.JSON(400, gin.H{"error": "document name required"})
		return
	}
	...
	if err != nil || res.MatchedCount == 0 {
		c.JSON(404, gin.H{"error": "Property not found or not owned by current user"})
		return
	}
...
func getVerificationQueue(c *gin.Context) {
	...
	if role != "AUTHORITY" && role != "ADMIN" {
		c.JSON(403, gin.H{"error": "Authority role required"})
		return
	}
	filter := bson.M{"verification_status": bson.M{"$in": []string{"SUBMITTED", "UNDER_REVIEW", "CONFLICT"}}}
...
func reviewVerification(c *gin.Context) {
	...
	decision := strings.ToUpper(req.Decision)
	if decision != "VERIFIED" && decision != "CONFLICT" && decision != "REJECTED" {
		c.JSON(400, gin.H{"error": "decision must be VERIFIED, CONFLICT or REJECTED"})
		return
	}
	status := decision
	res, err := db.Collection("properties").UpdateOne(context.Background(), bson.M{"property_id": c.Param("id")}, bson.M{"$set": bson.M{"verification_status": status, "verification_score": req.Score, "verification_notes": req.Notes, "verified_by": currentPhone(c), "verified_at": time.Now(), "updated_at": time.Now()}})
	if err != nil || res.MatchedCount == 0 {
		c.JSON(404, gin.H{"error": "Property not found"})
		return
	}
	c.JSON(200, gin.H{"status": status, "property_id": c.Param("id")})
}

func getRole(phone string) string {
	var u bson.M
	if db.Collection("users").FindOne(context.Background(), bson.M{"phone_number": phone}).Decode(&u) != nil {
		return "CITIZEN"
	}
...
func initiateTransfer(c *gin.Context) {
	...
	if c.BindJSON(&req) != nil || req.PropertyID == "" || req.BuyerPhone == "" {
		c.JSON(400, gin.H{"error": "property_id and buyer_phone required"})
		return
	}
	var prop bson.M
	if db.Collection("properties").FindOne(context.Background(), bson.M{"property_id": req.PropertyID}).Decode(&prop) != nil {
		c.JSON(404, gin.H{"error": "Property not found"})
		return
	}
	if prop["owner_phone"] != currentPhone(c) {
		c.JSON(403, gin.H{"error": "Only current owner can initiate transfer"})
		return
	}
	status, _ := prop["verification_status"].(string)
	if status != "VERIFIED" {
		c.JSON(409, gin.H{"error": "Transfer blocked: property is not VERIFIED", "verification_status": status})
		return
	}
	transferID := "TRF-" + strings.ToUpper(randomToken()[:10])
	t := bson.M{"transfer_id": transferID, "property_id": req.PropertyID, "seller_phone": currentPhone(c), "buyer_phone": req.BuyerPhone, "amount": req.Amount, "notes": req.Notes, "status": "PENDING", "created_at": time.Now()}
	if _, err := db.Collection("transfers").InsertOne(context.Background(), t); err != nil {
		c.JSON(500, gin.H{"error": "Could not create transfer"})
		return
	}
	c.JSON(201, t)
}

func listTransfers(c *gin.Context, field string) {
	cur, err := db.Collection("transfers").Find(context.Background(), bson.M{field: currentPhone(c)}, options.Find().SetSort(bson.D{{Key: "created_at", Value: -1}}))
	if err != nil {
		c.JSON(500, gin.H{"error": "Database error"})
		return
	}
	...
	var docs []bson.M
	_ = cur.All(context.Background(), &docs)
	c.JSON(200, gin.H{"transfers": docs, "count": len(docs)})
}
...
func acceptTransfer(c *gin.Context) {
	var t bson.M
	if db.Collection("transfers").FindOne(context.Background(), bson.M{"transfer_id": c.Param("id"), "buyer_phone": currentPhone(c), "status": "PENDING"}).Decode(&t) != nil {
		c.JSON(404, gin.H{"error": "Pending transfer not found"})
		return
	}
	pid := t["property_id"].(string)
	res, err := db.Collection("properties").UpdateOne(context.Background(), bson.M{"property_id": pid, "verification_status": "VERIFIED"}, bson.M{"$set": bson.M{"owner_phone": currentPhone(c), "updated_at": time.Now()}})
	if err != nil || res.MatchedCount == 0 {
		c.JSON(409, gin.H{"error": "Property is no longer verified or could not be transferred"})
		return
	}
	_, err = db.Collection("transfers").UpdateOne(context.Background(), bson.M{"transfer_id": c.Param("id"), "status": "PENDING"}, bson.M{"$set": bson.M{"status": "COMPLETED", "completed_at": time.Now()}})
	if err != nil {
		c.JSON(500, gin.H{"error": "Transfer updated partially; inspect transfer record"})
		return
	}
...
func rejectTransfer(c *gin.Context) {
	...
	_ = c.BindJSON(&req)
	res, err := db.Collection("transfers").UpdateOne(context.Background(), bson.M{"transfer_id": c.Param("id"), "buyer_phone": currentPhone(c), "status": "PENDING"}, bson.M{"$set": bson.M{"status": "REJECTED", "rejection_reason": req.Reason, "rejected_at": time.Now()}})
	if err != nil || res.MatchedCount == 0 {
		c.JSON(404, gin.H{"error": "Pending transfer not found"})
		return
	}

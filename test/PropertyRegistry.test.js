...
describe("PropertyRegistry", function () {
  ...
  it("registers a property without storing sensitive record data", async function () {
    ...
    await expect(registry.connect(owner).registerProperty("DHARANI-PROP-001"))
      .to.emit(registry, "PropertyRegistered")
      .withArgs(1n, "DHARANI-PROP-001", owner.address, await latestTimestamp(registry));
  ...
  it("allows only an authority to anchor a verification report", async function () {
    const { registry, authority, owner, outsider } = await deployFixture();
    ...
    await expect(registry.connect(authority).anchorVerification(1, reportHash))
      .to.emit(registry, "VerificationAnchored")
      .withArgs(1n, reportHash, authority.address, await latestTimestamp(registry));
  ...
  it("supports multiple verification anchors for the same property", async function () {
    const { registry, authority, owner } = await deployFixture();
    await registry.connect(owner).registerProperty("DHARANI-PROP-001");
  ...
  it("allows the verified owner to transfer the property", async function () {
    const { registry, authority, owner, buyer } = await deployFixture();
    await registry.connect(owner).registerProperty("DHARANI-PROP-001");
    ...
    await expect(registry.connect(owner).transferProperty(1, buyer.address))
      .to.emit(registry, "PropertyTransferred")
      .withArgs(1n, owner.address, buyer.address, await latestTimestamp(registry));
  ...
  async function latestTimestamp(contract) {
